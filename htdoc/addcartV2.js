...
button.addEventListener("click", async function () {
    ...
    try {
        const resp = await fetch('/check_session.php');
        const json = await resp.json();
        if (!json.logged_in) {
            // show a friendly prompt with sign in link
            if (confirm("You must sign in to add items to cart. Sign in now?")) {
                // redirect to login page; preserve redirect
        ...
        if (!confirm("Can't verify sign-in. Sign in now?")) return;
        window.location.href = "/login.php?redirect=" + encodeURIComponent(window.location.pathname);
        return;
    }

    // ... existing code to add item to localStorage or call cart API ...
});
